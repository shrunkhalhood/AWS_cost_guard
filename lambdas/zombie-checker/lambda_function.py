...
def lambda_handler(event, context):
    ...
    try:
        # Scan DynamoDB for incomplete shutdowns that are NOT already zombies
        response = table.scan(
            FilterExpression=
                Attr('shutdown_complete').eq(False) &
                Attr('zombie').not_exists()          # ← only check non-zombies
        )
        items = response.get('Items', [])
        print(f"Found {len(items)} incomplete non-zombie shutdowns")
        ...
        for item in items:
            ...
            print(f"{service_type} {service_id} stopping for {minutes_elapsed:.1f} mins")
            ...
            if minutes_elapsed > 10:
                ...
                # Update DynamoDB — mark as zombie so we never alert again
                table.update_item(
                    Key={
                ...
                # Trigger Lambda #3 to send alert email ONCE
...
def trigger_notifier(service_id, service_type, region, minutes_elapsed):
    """Call Lambda #3 to send zombie alert email ONCE"""
    try:
        payload = {
            'alert_type':      'ZOMBIE',
            'service_id':      service_id,
        ...
        print(f"Notifier triggered ONCE for zombie: {service_id}")
